...
Le panneau **Backoffice ROMs** du client egui expose l’association sans shell, uniquement aux comptes connectés `admin`. Il liste les ROMs non liées et ne propose que les jeux du même système ; l’association est écrite dans SQLite puis le cache JSON local est régénéré. La fiche détaillée d’un jeu affiche aussi ses ROMs liées et permet leur désassociation. Aucun chemin de ROM ni fichier NAS n’est déplacé, renommé ou supprimé.
